    ...
    def run(self):
        while not rospy.is_shutdown():
            try:
                ...
                (trans, rot) = self.listener.lookupTransform('thorvald_001/odom', 'thorvald_001/kinect2_rgb_optical_frame', rospy.Time())
                ...
                continue
            '''
            #print base link postion on map
            t = self.listener.getLatestCommonTime("thorvald_001/base_link", "/map")
            (position, quaternion) = self.listener.lookupTransform('thorvald_001/base_link', '/map', t)
            print "base_link position on map is"
            print position, quaternion
            '''
            # the quick hack to go from quaternions to 2D angles:
            yaw_angle = atan2(rot[2], rot[3])*2
            print("the transformation from current position to kinect 2 is: ", trans, yaw_angle * 180 / pi)
            '''
