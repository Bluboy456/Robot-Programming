...
class move_and_turn:
    ...
    def __init__(self):
        ...
        self.listener = tf.TransformListener()
          #wait for tf messages to be published
        self.listener.waitForTransform('thorvald_001/hokuyo', 'thorvald_001/base_link', rospy.Time(), rospy.Duration(3))
        (self.trans_laser_to_base, self.rot_laser_to_base) = self.listener.lookupTransform('thorvald_001/hokuyo', 'thorvald_001/base_link', rospy.Time())
        rospy.spin()
    ...
    def callback (self, msg):      # called when a laser message arrives, extracts front, left and right distances
        ...
        #calculate obstacle postion relative to base to base_link (ignore z)
        #print ('laser to base transform is: ')    #DEBUG
        #print (self.trans_laser_to_base) #DEBUG

        self.obstacle_to_base_x = self.trans_laser_to_base[0] - self.obstacle_x
        self.obstacle_to_base_y = self.trans_laser_to_base[1] - self.obstacle_y
